...
class TestMattermostAllowedChannels:
    """Mattermost whitelist logic — replicated since the adapter reads env
    vars inline inside _handle_post rather than through a helper method."""

    @staticmethod
    def _would_process(channel_id, channel_type="O", allowed_env="", require_mention="true"):
        import os as _os
        # Replicate the gate from gateway/platforms/mattermost.py
        if channel_type == "D":
            return True
        if allowed_env:
            ...
            if channel_id not in allowed:
                return False
        # (require_mention logic intentionally omitted — whitelist runs first)
        return True

    def test_empty_is_no_restriction(self):
        assert self._would_process("chan123", allowed_env="") is True

    def test_blocks_non_whitelisted_channel(self):
        assert self._would_process("chanXYZ", allowed_env="chanABC,chanDEF") is False

    def test_permits_whitelisted_channel(self):
        assert self._would_process("chanABC", allowed_env="chanABC,chanDEF") is True

    def test_dm_unaffected(self):
        """DM channels (type='D') bypass the whitelist."""
        assert self._would_process("chanXYZ", channel_type="D", allowed_env="chanABC") is True
